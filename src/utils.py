...
def apply_spatial_zone_composition(mat_canvas: np.ndarray, rng: np.random.Generator, mat_size_px: float = 2600.0, strip_width_px: float = 320.0) -> np.ndarray:
    """
    Applies spatial zone composition natively:
    Combines dense array Mat Zones (DRAM contacts / FinFET gates) with Strip Routing Zones (power rails / interconnects)
    to create boundary edges that break periodic pattern ambiguity.
    """
    h, w = mat_canvas.shape[:2]
    strip_canvas = np.full((h, w), 95, dtype=np.uint8)
    half_w = 9.0 / 2.0
    pitch = 220.0

    for axis_pos, is_row in [
        (np.arange(float(rng.uniform(0, pitch)), float(w), pitch), True),
        (np.arange(float(rng.uniform(0, pitch)), float(h), pitch), False)
    ]:
        for center in axis_pos:
            lo = max(int(round(center - half_w)), 0)
            hi = min(int(round(center + half_w)), w if is_row else h)
            if is_row:
                strip_canvas[lo:hi, :] = 128
            else:
                strip_canvas[:, lo:hi] = 128

    combined = strip_canvas.copy()

    pos_x = 0.0
    is_mat_x = True
    x_spans = []
    while pos_x < w:
        span_len = mat_size_px if is_mat_x else strip_width_px
        end_x = min(pos_x + span_len, float(w))
        x_spans.append((is_mat_x, int(round(pos_x)), int(round(end_x))))
        pos_x = end_x
        is_mat_x = not is_mat_x

    pos_y = 0.0
    is_mat_y = True
    y_spans = []
    while pos_y < h:
        span_len = mat_size_px if is_mat_y else strip_width_px
        end_y = min(pos_y + span_len, float(h))
        y_spans.append((is_mat_y, int(round(pos_y)), int(round(end_y))))
        pos_y = end_y
        is_mat_y = not is_mat_y

    for is_m_y, y0, y1 in y_spans:
        for is_m_x, x0, x1 in x_spans:
            if is_m_y and is_m_x:
                combined[y0:y1, x0:x1] = mat_canvas[y0:y1, x0:x1]
...
def render_composite_field(pattern_type, size, fin_pitch, gate_pitch, difficulty, rng, debug_layers=False):
    ...
    if pattern_type in ["FIN_ARRAY", "FIN_CUT", "FIN_GATE"]:
        ...
        if pattern_type == "FIN_CUT":
            ...
            # Debug print suppressed for clean console output
            pass
...
def render_fine_dram_mesh_mat(w: int, h: int, pitch_x: float, pitch_y: float, rng: np.random.Generator) -> np.ndarray:
    canvas = np.full((h, w), 45, dtype=np.uint8)
    line_w = max(2, int(round(pitch_x * 0.14)))
    
    for x in np.arange(pitch_x * 0.5, w, pitch_x):
        cx = int(round(x))
        cv2.line(canvas, (cx, 0), (cx, h), 175, line_w, cv2.LINE_AA)
        
    for y in np.arange(pitch_y * 0.5, h, pitch_y):
        cy = int(round(y))
        cv2.line(canvas, (0, cy), (w, cy), 195, line_w, cv2.LINE_AA)
...
def maybe_collapse_gap(gap_nm: float, collapse_threshold_nm: float, rng: np.random.Generator) -> bool:
    if collapse_threshold_nm <= 0.0:
        return False
    if gap_nm <= collapse_threshold_nm:
        prob = max(0.0, min(1.0, 1.0 - (gap_nm / collapse_threshold_nm)))
...
def generate_dram_canvas_verbatim(
    size_px: int,
    preset: dict,
    collapse_threshold_nm: float,
    rng: np.random.Generator,
    ...
    canvas = np.full((size_px, size_px), BACKGROUND_DRAM, dtype=np.uint8)
...
DRAM_HF_PRESETS = [
    {"name": "dram_1x",      "word_line_pitch_nm": 64,  "word_line_width_nm": 32, "bit_line_pitch_nm": 96,  "bit_line_width_nm": 32, "contact_diameter_nm": 32},
    {"name": "dram_dense",   "word_line_pitch_nm": 48,  "word_line_width_nm": 24, "bit_line_pitch_nm": 72,  "bit_line_width_nm": 24, "contact_diameter_nm": 24},
    {"name": "dram_loose",   "word_line_pitch_nm": 96,  "word_line_width_nm": 48, "bit_line_pitch_nm": 144, "bit_line_width_nm": 48, "contact_diameter_nm": 48},
    {"name": "dram_wide",    "word_line_pitch_nm": 120, "word_line_width_nm": 56, "bit_line_pitch_nm": 180, "bit_line_width_nm": 60, "contact_diameter_nm": 58},
    {"name": "dram_compact", "word_line_pitch_nm": 72,  "word_line_width_nm": 30, "bit_line_pitch_nm": 108, "bit_line_width_nm": 34, "contact_diameter_nm": 30},
    {"name": "dram_legacy",  "word_line_pitch_nm": 160, "word_line_width_nm": 78, "bit_line_pitch_nm": 240, "bit_line_width_nm": 80, "contact_diameter_nm": 78},
]
...
def render_16_mats_field_fine(
    ...
    for r_idx, (row_is_mat, y0, y1) in enumerate(row_spans):
        for c_idx, (col_is_mat, x0, x1) in enumerate(col_spans):
            if row_is_mat and col_is_mat and y1 > y0 and x1 > x0:
                ...
                if pattern_type == "DRAM_CELL":
                    preset = DRAM_HF_PRESETS[mat_idx % len(DRAM_HF_PRESETS)]
                    mat_img = generate_dram_canvas_verbatim(mat_w, preset, 0.0, mat_rng)
                    cv2.rectangle(mat_img, (0, 0), (mat_w - 1, mat_h - 1), 25, 12)
                    canvas[y0:y1, x0:x1] = mat_img[:mat_h, :mat_w]
                elif pattern_type == "GATE_POLY":
    ...
    if pattern_type == "DRAM_CELL":
        downsampled = cv2.resize(canvas, (size_px, size_px), interpolation=cv2.INTER_AREA)
    else:
...
def render_dram_cell_field(size_px: int = 1000, rng: np.random.Generator = None) -> np.ndarray:
    """
    Renders 2D Staggered Honeycomb DRAM Memory Cell Array with active capacitor pillars & routing zones.
    """
    return render_16_mats_field_fine("DRAM_CELL", size_px=size_px, rng=rng)
